...
const mockMenu: object[] = [
  {
    name: 'Elements',
    link: 'element',
    glyph: 'fa fa-fw fa-desktop',
    children: [],
    subMenuIndex: 0
  },
  {
...
describe('SidebarComponent', () => {
  ...
  it('should be created initially with just the hard coded items', () => {
    ...
    const defFirstName = component.hardCodedItems[0];
    const compListLength = component.menuList.length;
    const compFirstName = component.menuList[0];
    expect(compListLength).toEqual(
      deflistLength,
      'menu list not instantiated correctly'
    );
    expect(compFirstName).toEqual(
      defFirstName,
      `first item name doesn't equal ${defFirstName}`
    );
  });
  ...
  it('should create the menu with combined hard coded items and remote items when subscribed to with a goodUrl', () => {
    ...
    component.remoteMenuList = mockMenu;
    component.mergeMenus();

    const combListLength =
      component.hardCodedItems.length + mockMenu.length;
    const combFirstName = mockMenu[0];
    const compListLength = component.menuList.length;
    const compFirstName = component.menuList[0];
    expect(compListLength).toEqual(
      combListLength,
    ...
    expect(compFirstName).toEqual(
      combFirstName,
      `first item name doesn't equal ${combFirstName}`
    );
  });
});
